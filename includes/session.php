<?php
    ...
    function registerCustomer($first_name, $last_name, $email_address, $password, $no_handphone){
        ...
        if(getCustomer($email_address) == null){
            ...
            $query = "INSERT INTO customer (id_customer, first_name, last_name, email_address, password, handphone_number, digital_certificate) VALUES (?, ?, ?, ?, ?, ?,?)";
			      $myNull = null;
            if($insert_stmt = $core->dbh->prepare($query)){
                ...
                $insert_stmt->bindParam('6', $no_handphone);
				        $insert_stmt->bindParam('7', $myNull);
    ...
    function registerMerchant($merchant_name, $email_address, $password, $address, $no_handphone){
        ...
        if(getMerchant($email_address) == null){
            ...
            $query = "INSERT INTO merchant (id_merchant, merchant_name, email_address, password, address, handphone_number, digital_certificate) VALUES (?, ?, ?, ?, ?, ?,?)";
            if($insert_stmt = $core->dbh->prepare($query)){
                $insert_stmt->bindParam('1', $id_merchant);
                $insert_stmt->bindParam('2', $merchant_name);
                $insert_stmt->bindParam('3', $email_address);
                $insert_stmt->bindParam('4', $hashed_password);
                $insert_stmt->bindParam('5', $address);
                $insert_stmt->bindParam('6', $no_handphone);
				        $insert_stmt->bindParam('7', $myNull);
    ...
    function getMerchant($email_address){
        ...
        if ($stmt = $core->dbh->prepare("SELECT id_merchant, merchant_name, email_address, password, address, handphone_number, digital_certificate FROM merchant WHERE email_address = :email_address LIMIT 1")) {
            ...
            if($stmt->rowCount() == 1){
                ...
                $password = $row[3];
                $address = $row[4];
                $handphone_number = $row[5];
                $digital_certificate = $row[6];

                $merchant = new Merchant($id_merchant, $merchant_name, $email_address, $password, $address, $handphone_number, $digital_certificate);
                return $merchant;
            }
            else{
                return null;
    ...
    function getCustomer($email_address){
        ...
        if ($stmt = $core->dbh->prepare("SELECT id_customer, first_name, last_name, email_address, password, handphone_number, digital_certificate FROM customer WHERE email_address = :email_address LIMIT 1")) {
            ...
            if($stmt->rowCount() == 1){
                ...
                $customer = new Customer($id_customer, $first_name, $last_name, $email_address, $password, $handphone_number, $digital_certificate);
                return $customer;
            }
            else{
                return null;
